...
\title{SCCM 2017 Abstract Submission: In-Hospital and 3-Year Mortality Among TBI Patients with $\ge$ Day in ICU}
...
betablock.plot <- ggplot(data = betablock.predict, aes(x = tot.betablock.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.5) +
...
Tables \ref{table:basestats} and \ref{table:inhosp.stats} present baseline and in-hospital
