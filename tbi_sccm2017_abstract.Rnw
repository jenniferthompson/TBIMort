...
<<datamgmt>>=
## -- Relevel CPR; original labels are very long, cause formatting issues --------------------------
...
create.results.matrix <- function(modobj){
  ...
  paste.quants <- function(var, inc.pcts = FALSE){
    tmp <- paste0(rndformat(dd$limits[[var]][3]), ' vs. ', rndformat(dd$limits[[var]][1]))
    if(inc.pcts){
      tmp <- paste0('~~~', tmp, ' (', pct.adjto[2]*100, 'th vs ', pct.adjto[1]*100, 'th pctile)')
    }
  ...
  p.opioid <- anova(modobj)['tot.opioid.cube', 'P']
  p.benzo <- anova(modobj)['tot.benzo.cube', 'P']
  p.antipsyc <- anova(modobj)['tot.antipsyc.cube', 'P']
  p.propofol <- anova(modobj)['tot.propofol.cube', 'P']
  p.dex <- anova(modobj)['tot.dex.cube', 'P']
  p.pento <- anova(modobj)['tot.pento.cube', 'P']
  p.clonid <- anova(modobj)['tot.clonid.cube', 'P']
  p.insurance <- anova(modobj)['insurance.code', 'P']
  p.vec <- c(formatp(p.mental), '', '', formatp(p.betablock), '', formatp(p.opioid), formatp(p.benzo),
             formatp(p.antipsyc), formatp(p.propofol), formatp(p.dex), formatp(p.pento),
             formatp(p.clonid), formatp(p.insurance), '', '', '')
  ...
  hrci.opioid <- hr.ci.string(summary.modobj[grep('opioid', rownames(summary.modobj)) + 1,])
  hrci.benzo <- hr.ci.string(summary.modobj[grep('benzo', rownames(summary.modobj)) + 1,])
  hrci.antipsyc <- hr.ci.string(summary.modobj[grep('antipsyc', rownames(summary.modobj)) + 1,])
  hrci.propofol <- hr.ci.string(summary.modobj[grep('propofol', rownames(summary.modobj)) + 1,])
  hrci.dex <- hr.ci.string(summary.modobj[grep('dex', rownames(summary.modobj)) + 1,])
  hrci.pento <- hr.ci.string(summary.modobj[grep('pento', rownames(summary.modobj)) + 1,])
  hrci.clonid <- hr.ci.string(summary.modobj[grep('clonid', rownames(summary.modobj)) + 1,])
  hrci.ins.public <- hr.ci.string(summary.modobj[grep('Public', rownames(summary.modobj)) + 1,])
  hrci.ins.none <- hr.ci.string(summary.modobj[grep('Self Pay', rownames(summary.modobj)) + 1,])
  hrci.ins.wc <- hr.ci.string(summary.modobj[grep('Workers Comp', rownames(summary.modobj)) + 1,])
  hr.vec <- c('', hrci.del, hrci.coma, '', hrci.betablock, hrci.opioid, hrci.benzo, hrci.antipsyc,
              hrci.propofol, hrci.dex, hrci.pento, hrci.clonid, '', hrci.ins.public, hrci.ins.none,
              hrci.ins.wc)
  ...
  rownames(results.matrix) <- c('Mental status', '~~~Delirium vs normal', '~~~Coma vs normal',
                                ...
                                paste.quants('tot.betablock.cube', TRUE),
                                paste0('24h opioids, cube root; ', paste.quants('tot.opioid.cube')),
                                paste0('24h benzos, cube root; ', paste.quants('tot.benzo.cube')),
                                paste0('24h antipsychotics, cube root; ',
                                       paste.quants('tot.antipsyc.cube')),
                                paste0('24h propofol, cube root; ',
                                       paste.quants('tot.propofol.cube')),
                                paste0('24h dex, cube root; ', paste.quants('tot.dex.cube')),
                                paste0('24h pentobarbital, cube root; ',
                                       paste.quants('tot.pento.cube')),
                                paste0('24h clonidine, cube root; ', paste.quants('tot.clonid.cube')),
                                'Insurance', '~~~Public vs private',
                                '~~~Self-pay or none vs private',
                                '~~~Workers comp vs private')
...
## -- Create four-facet figures showing association between drugs and hazard of death --------------
mortality.levels <- c('In-Hospital Mortality', '3-Year Mortality')
icp.levels <- c('ICP, original scale', 'ICP, normal vs abnormal')
...
betablock.predict <- do.call(rbind,
                             lapply(1:length(mod.list),
                                    FUN = function(m){
                                      ...
                                      tmp$time <- time.icp[2]
                                      tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                                     levels = 1:2,
                                                                     labels = mortality.levels))
                                      tmp$icp <- time.icp[3]
                                      tmp$icp.f <- with(tmp, factor(ifelse(icp == 'icpc', 1, 2),
                                                                    levels = 1:2,
                                                                    labels = icp.levels))
                                      tmp
                                    }))
...
opioid.predict <- do.call(rbind,
                          lapply(1:length(mod.list),
                                 FUN = function(m){
                                   tmp <- as.data.frame(Predict(mod.list[[m]],
                                                                tot.opioid.cube = NA,
                                                                fun = exp))
                                   tmp <- tmp[,c('tot.opioid.cube', 'yhat', 'lower', 'upper')]
                                   time.icp <- strsplit(names(mod.list)[m], '\\.')[[1]]
                                   tmp$time <- time.icp[2]
                                   tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                                  levels = 1:2,
...
opioid.plot <- ggplot(data = opioid.predict, aes(x = tot.opioid.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.4) +
  geom_line(colour = 'navy') +
  facet_wrap(icp.f ~ time.f, scales = 'free') +
  xlab('24h Opioids, Cube Root') +
  ylab('Hazard of Death')

benzo.predict <- do.call(rbind,
                         lapply(1:length(mod.list),
                                FUN = function(m){
                                  tmp <- as.data.frame(Predict(mod.list[[m]],
                                                               tot.benzo.cube = NA,
                                                               fun = exp))
                                  tmp <- tmp[,c('tot.benzo.cube', 'yhat', 'lower', 'upper')]
                                  time.icp <- strsplit(names(mod.list)[m], '\\.')[[1]]
                                  tmp$time <- time.icp[2]
                                  tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                                 levels = 1:2,
...
benzo.plot <- ggplot(data = benzo.predict, aes(x = tot.benzo.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.4) +
  geom_line(colour = 'navy') +
  facet_wrap(icp.f ~ time.f, scales = 'free') +
  xlab('24h Benzos, Cube Root') +
  ylab('Hazard of Death')

antipsyc.predict <- do.call(rbind,
                            lapply(1:length(mod.list),
                                   FUN = function(m){
                                     tmp <- as.data.frame(Predict(mod.list[[m]],
                                                                  tot.antipsyc.cube = NA,
                                                                  fun = exp))
                                     tmp <- tmp[,c('tot.antipsyc.cube', 'yhat', 'lower', 'upper')]
                                     time.icp <- strsplit(names(mod.list)[m], '\\.')[[1]]
                                     tmp$time <- time.icp[2]
                                     tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                                    levels = 1:2,
...
antipsyc.plot <- ggplot(data = antipsyc.predict, aes(x = tot.antipsyc.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.4) +
  geom_line(colour = 'navy') +
  facet_wrap(icp.f ~ time.f, scales = 'free') +
  xlab('24h Antipsychotics, Cube Root') +
  ylab('Hazard of Death')

propofol.predict <- do.call(rbind,
                            lapply(1:length(mod.list),
                                   FUN = function(m){
                                     tmp <- as.data.frame(Predict(mod.list[[m]],
                                                                  tot.propofol.cube = NA,
                                                                  fun = exp))
                                     tmp <- tmp[,c('tot.propofol.cube', 'yhat', 'lower', 'upper')]
                                     time.icp <- strsplit(names(mod.list)[m], '\\.')[[1]]
                                     tmp$time <- time.icp[2]
                                     tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                                    levels = 1:2,
...
propofol.plot <- ggplot(data = propofol.predict, aes(x = tot.propofol.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.4) +
  geom_line(colour = 'navy') +
  facet_wrap(icp.f ~ time.f, scales = 'free') +
  xlab('24h Propofol, Cube Root') +
  ylab('Hazard of Death')

dex.predict <- do.call(rbind,
                       lapply(1:length(mod.list),
                              FUN = function(m){
                                tmp <- as.data.frame(Predict(mod.list[[m]],
                                                             tot.dex.cube = NA,
                                                             fun = exp))
                                tmp <- tmp[,c('tot.dex.cube', 'yhat', 'lower', 'upper')]
                                time.icp <- strsplit(names(mod.list)[m], '\\.')[[1]]
                                tmp$time <- time.icp[2]
                                tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                               levels = 1:2,
...
dex.plot <- ggplot(data = dex.predict, aes(x = tot.dex.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.4) +
  geom_line(colour = 'navy') +
  facet_wrap(icp.f ~ time.f, scales = 'free') +
  xlab('24h Dexmedetomidine, Cube Root') +
  ylab('Hazard of Death')

clonid.predict <- do.call(rbind,
                          lapply(1:length(mod.list),
                                 FUN = function(m){
                                   tmp <- as.data.frame(Predict(mod.list[[m]],
                                                                tot.clonid.cube = NA,
                                                                fun = exp))
                                   tmp <- tmp[,c('tot.clonid.cube', 'yhat', 'lower', 'upper')]
                                   time.icp <- strsplit(names(mod.list)[m], '\\.')[[1]]
                                   tmp$time <- time.icp[2]
                                   tmp$time.f <- with(tmp, factor(ifelse(time == 'inhosp', 1, 2),
                                                                  levels = 1:2,
...
clonid.plot <- ggplot(data = clonid.predict, aes(x = tot.clonid.cube, y = yhat)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = 'navy', alpha = 0.4) +
  geom_line(colour = 'navy') +
  facet_wrap(icp.f ~ time.f, scales = 'free') +
  xlab('24h Clonidine, Cube Root') +
  ylab('Hazard of Death')
...
<<betablockplot, results = 'asis', fig.cap = '24-Hour Beta-Blocker Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
betablock.plot

@

<<opioidkplot, results = 'asis', fig.cap = '24-Hour Opioid Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
opioid.plot

@

<<benzoplot, results = 'asis', fig.cap = '24-Hour Benzodiazepine Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
benzo.plot

@

<<antipsycplot, results = 'asis', fig.cap = '24-Hour Antipsychotic Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
antipsyc.plot

@

<<propofolplot, results = 'asis', fig.cap = '24-Hour Propofol Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
propofol.plot

@

<<dexplot, results = 'asis', fig.cap = '24-Hour Dexmedetomidine Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
dex.plot

@

<<clonidplot, results = 'asis', fig.cap = '24-Hour Clonidine Dose vs Mortality', fig.align = 'center', fig.pos = '!h', fig.height=4.5, fig.width = 7>>=
clonid.plot
