...
source(file.path(home.dir, 'thomps23', 'R', 'multiplot.r'))

## Source function for creating time-varying data sets for Cox models
source('tbi_timevarying.R')

## Source update to as.mids() *not* requiring data set to be sorted by .imp
source(file.path(home.dir, 'thomps23', 'R', 'JTMisc', 'as_mids_update.R'))

## Source functions for getting results from rms model fits
source(file.path(home.dir, 'thomps23', 'R', 'rmsHelpers', 'model_results.R'))
source(file.path(home.dir, 'thomps23', 'R', 'rmsHelpers', 'calc_comparisons_rms.R'))
...
mort.corr.data <- select(tbi.oneobs.icu, mrn, one_of(mortality.continuous.covars)) %>%
  right_join(select(tbi.daily.icu, mrn, one_of(mortality.continuous.covars)),
             by = 'mrn')

mort.inhosp.corr.data <- mort.corr.data %>%
  filter(mrn %in% pts.inc) %>%
  select(-mrn, -vent.days, -los)

mort.3yr.corr.data <- mort.corr.data %>%
  filter(mrn %in% pts.inc.survived) %>%
  select(-mrn)

## Continuous vs continuous: Spearman correlations
library(corrplot)
corr.mort.inhosp <- cor(mort.inhosp.corr.data, use = 'pairwise.complete.obs', method = 'spearman')
...
```{r print_mort_3yr_corr, results = 'asis'}
corrplot.mixed(corr.mort.3yr,
               tl.cex = 0.5, title = 'Three-Year Mortality', tl.col = 'black', mar = c(0, 0, 1, 0))
...
base.mort.3yr <- tbi.oneobs.icu %>%
  ...
  select(mrn, age, gender, insurance.code, iss, cpr.yn, marshall.comb, pt.cerebral.na,
         disposition.comb, vent.days, los, died.3yr.adm, time.death.3yr.adm)
...
tv.mort.3yr <- timevarying.data(idvar = 'mrn',
                                ...
                                time.var = 'time.death.3yr.adm',
                                event.var = 'died.3yr.adm',
                                event.string = 'Yes',
                                data.set = tbi.oneobs.icu,
                                out.strings = c('Alive through today', 'Died today'))
...
modeldata.mort.3yr <- tv.mort.3yr %>%
  ...
  mutate(max.motor.imp = set.backto.na(max.motor.imp),
         ...
         mental.status = factor(set.backto.na(mental.status),
                                levels = 1:3, labels = levels(tbi.daily$mental.status)),
         ## If start.day = stop.day, add 0.5 to stop.day (mostly occurs in patients discharged to
         ## hospice who died on day of discharge)
         stop.day = ifelse(stop.day == start.day, stop.day + 0.5, stop.day)) %>%
  ## Add on baseline variables
...
areg.mort.3yr <- aregImpute(~ age + gender + insurance.code + iss + cpr.yn + marshall.comb +
                              pt.cerebral.na + died.3yr.adm + I(max.motor.imp) + pupil.react.imp +
                              min.glucose.imp + min.hemoglobin.imp + min.sodium.imp +
                              mental.status + sofa.mod.nanormal.imp + max.icp.dich +
                              tot.opioid.cube + tot.benzo.cube + tot.antipsyc.cube +
                              tot.betablock.cube + tot.propofol.cube + any.dex +
...
`r figure_nums('mort_3yr_zph', display = 'cite')` graphically examines the proportional hazards
assumption (that beta(t) is roughly constant over time; in other words, we want to see flat lines).

#### `r figure_nums('mort_3yr_zph')`
```{r mort_3yr_checkph, results='asis'}
## -- Check proportional hazard assumption ---------------------------------------------------------
...
```{r mort_3yr_results}
dd.3yr <- datadist(modeldata.mort.3yr, adjto.cat = 'first')
options(datadist = 'dd.3yr')
results.mort.3yr <- model.results(mod.mort.3yr,
                                  data.set = modeldata.mort.3yr,
                                  print.format = 'plain')
...
pandoc.table(results.mort.3yr, justify = 'lrrrrrr', style = 'rmarkdown', split.tables = 800)
```
