...
```{r mort_inhosp_figures, results = 'asis', fig.width=10, fig.height=50}
...
## Work on factor levels
hrs.mort.inhosp.flevels <- unique(subset(hrs.mort.inhosp, select = c(comp.c, is.ref))) %>%
...
gg.mort.inhosp <- ggplot(data = hrs.mort.inhosp,
                         aes(x = flevel, label = ref.c, y = effect)) +
  facet_wrap(~ variable, ncol = 1, scales = 'free') +
  geom_hline(yintercept = 1, colour = 'grey80', size = 1.25) +
  geom_pointrange(aes(ymin = lcl, ymax = ucl, colour = abs(log(effect)))) +
  scale_color_viridis(direction = -1, end = 0.9, name = 'Effect Size<br>(Log Scale)') +
  scale_x_discrete(name = '') +
  scale_y_continuous(name = 'Hazard Ratios (95% Confidence Intervals), In-Hospital Mortality<br> ',
