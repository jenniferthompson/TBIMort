...
the [same covariates](#mort3yrcovariates) as the three-year mortality model. Please see [that
section](#mort3yrmissingness) for details on covariate missingness. Among survivors, `r pctMissFIM`%
(N = `r nMissFIM`) were missing FIM score at hospital discharge. Missing covariates and outcomes
will be handled using multiple imputation.

## Model Choice

The distribution of FIM scores at discharge, shown below, follows a distribution not described by a
mathematical distribution; therefore, we will use a proportional odds logistic regression model with
multiple imputation to look for relationships between it and baseline risk factors. POLR models are
robust to assumptions of outcome distribution.
