...
complete.mort.3yr <- complete(mice.mort.3yr, action = 'long', include = TRUE)
...
mort.3yr.corr.data <- summary.mort.3yr %>%
  filter(.imp == '1') %>%
  select(-.imp, -mrn, -days.no.pupil.data, -ever.dex, -ever.prbc, -ever.plasmaplt, -gender,
         -insurance.code, -cpr.yn, -marshall.comb, -pt.cerebral.na, -disposition.comb, -died.3yr.dc,
         -time.death.3yr.dc)
...
hrs.mort.3yr <- hrs.mort.3yr %>%
  ...
  left_join(select(results.mort.3yr, Covariate, P),
            by = c("varlabel" = "Covariate"))
...
## Merge numeric values onto original data set, create final factor variable
hrs.mort.3yr <- hrs.mort.3yr %>%
  left_join(select(hrs.mort.3yr.flevels, comp.c, flevel.num),
            by = c('comp.c')) %>%
