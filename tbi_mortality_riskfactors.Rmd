...
```{r mort_inhosp_figures, results = 'asis', fig.width=10, fig.height=15}
...
gg.mort.inhosp <- ggplot(data = hrs.mort.inhosp, aes(x = comp, y = effect)) +
  facet_wrap(~ variable, ncol = 3, scales = 'free_x') +
  geom_hline(yintercept = 1, colour = 'grey80', size = 1.5) +
  geom_pointrange(aes(ymin = lcl, ymax = ucl), colour = '#003D74') +
  scale_x_continuous(name = '') +
  scale_y_continuous(name = 'Hazard Ratio (95% Confidence Interval)',
                     breaks = c(0, 0.25, 0.5, 1, 1.5, 2, 4, 8, 16)) +
  # scale_shape(guide = FALSE) +
  # scale_y_continuous(trans = 'log', breaks = c(0, 0.25, 0.5, 1, 2, 4, 8, 16, 32)) +
  theme_bw()

ggplotly(gg.mort.inhosp)
