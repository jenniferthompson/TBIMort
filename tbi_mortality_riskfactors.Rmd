...
summaryM(mental.status + max.motor.imp + pupil.react.imp + min.glucose.imp + min.hemoglobin.imp +
           ...
           tot.clonid + any.plasmaplt + units.cryo + units.plasma + units.platelets +
           units.prbc ~ 1,
         data = tbi.daily.icu)
...
daily.drugs <- tbi.daily.icu %>%
  select(mrn, study.day, tot.opioid.cube:units.cryo) %>%
  gather(key = drug.or.product, value = amount, tot.opioid.cube:units.cryo)
...
areg.mort.inhosp <- aregImpute(~ age + gender + insurance.code + iss + cpr.yn + pt.marshall +
                                 ...
                                 tot.betablock.cube + tot.propofol.cube + tot.dex.cube +
                                 tot.clonid.cube + units.prbc + any.plasmaplt + start.day,
...
mod.mort.inhosp <- fit.mult.impute(Surv.mort.inhosp ~ rcs(age, 4) + gender + insurance.code +
                                     ...
                                     max.icp.dich + rcs(tot.opioid.cube, 4) +
                                     rcs(tot.benzo.cube, 4) + rcs(tot.antipsyc.cube, 3) +
                                     rcs(tot.betablock.cube, 4) + rcs(tot.propofol.cube, 4) +
                                     rcs(tot.dex.cube, 3) + rcs(tot.clonid.cube, 3) +
                                     rcs(units.prbc, 3) + any.plasmaplt,
                                   fitter = cph,
                                   xtrans = areg.mort.inhosp,
                                   data = modeldata.mort.inhosp,
                                   x = TRUE, y = TRUE)

dd.mort.inhosp <- datadist(modeldata.mort.inhosp)
options(datadist = 'dd.mort.inhosp', width = 150)
summary(mod.mort.inhosp)
