...
table_nums("deldays_missing", caption = "Missingness for Delirium Duration Model")
table_nums(name = 'deldays_results', caption = 'Model Results Summary, Delirium Duration')
table_nums("dcfds_missing", caption = "Missingness for Delirium/Coma-Free Days Model")
table_nums(name = 'dcfds_results', caption = 'Model Results Summary, Delirium/Coma-Free Days')
table_nums("fim_missing", caption = "Missingness for FIM Model")
table_nums(name = 'fim_results', caption = 'Model Results Summary, FIM at Discharge')
table_nums(name = 'hospdc_results', caption = 'Model Results Summary, Time to Hospital Discharge')
...
figure_nums(name = 'dcfds_figures', 'Odds Ratios and 95% CIs, Delirium/Coma-Free Days')
figure_nums(name = 'fim_figures', 'Odds Ratios and 95% CIs, Functional Independence Measure at Discharge')
figure_nums(name = 'hospdc_figures', 'Hazard Ratios and 95% CIs, Time to Hospital Discharge')
...
## Ever/never and "days among exposed" for delirium, coma, mental status
tbi.oneobs <- tbi.oneobs %>%
  mutate(ever.del = factor(ifelse(is.na(days.del), NA,
                           ifelse(days.del == 0, 0, 1)),
                           levels = 0:1, labels = c('Never', 'At least one day')),
         days.del.exp = ifelse(is.na(ever.del) | (!is.na(ever.del) & ever.del == 'Never'), NA,
                               days.del),
         ever.del.14 = factor(ifelse(is.na(days.del.14), NA,
                              ifelse(days.del.14 == 0, 0, 1)),
                              levels = 0:1, labels = c('Never', 'At least one day')),
         days.del.14.exp = ifelse(is.na(ever.del.14) |
                                    (!is.na(ever.del.14) & ever.del.14 == 'Never'), NA,
                                  days.del.14),
         ever.coma = factor(ifelse(is.na(days.coma), NA,
                            ifelse(days.coma == 0, 0, 1)),
                            levels = 0:1, labels = c('Never', 'At least one day')),
         days.coma.exp = ifelse(is.na(ever.coma) | (!is.na(ever.coma) & ever.coma == 'Never'), NA,
                                days.coma),
         ever.mental = factor(ifelse(is.na(days.mental), NA,
                              ifelse(days.mental == 0, 0, 1)),
                              levels = 0:1, labels = c('Never', 'At least one day')),
         days.mental.exp = ifelse(is.na(ever.mental) |
                                    (!is.na(ever.mental) & ever.mental == 'Never'), NA,
                                  days.mental))

label(tbi.oneobs$ever.del) <- 'Ever delirious?'
label(tbi.oneobs$days.del.exp) <- 'Days of delirium among exposed'
label(tbi.oneobs$ever.del.14) <- 'Ever delirious within 14 days of admission?'
label(tbi.oneobs$days.del.14.exp) <- 'Days of delirium within 14 days among exposed'
label(tbi.oneobs$ever.coma) <- 'Ever comatose?'
label(tbi.oneobs$days.coma.exp) <- 'Days of coma among exposed'
label(tbi.oneobs$ever.mental) <- 'Ever assessed for mental status?'
label(tbi.oneobs$days.mental.exp) <- 'Days of mental status info among those assessed'
...
## -- Patients who survived hospital stay -> eligible for 3-year mortality, delirium duration, -----
## -- FIM models -----------------------------------------------------------------------------------
pts.inc.survived <- subset(tbi.oneobs.icu, hosp.death == 'No')$mrn
tbi.survivors <- subset(tbi.oneobs.icu, hosp.death == 'No')
...
html(summaryM(age + gender + race + insurance.code + iss + cpr.yn + pt.marshall + pt.cerebral +
                pt.epidural + pt.injury + vent.days + ever.mental + days.mental + days.mental.exp +
                ever.del + days.del + days.del.exp + ever.del.14 + days.del.14 + days.del.14.exp +
                ever.coma + days.coma + days.coma.exp + dcfd.14 + icu.los + los + hosp.death +
                disposition.coded + fim.total + died.3yr.adm ~ 1,
              data = tbi.oneobs.icu),
...
### Model Covariates {#mortinhospcovariates}
...
- Daily In-Hospital (all per 24h)
    ...
    - Alpha-2 agonists: dichotomous indicator for whether patient received clonidine and/or dexmedetomidine in a 24-hour period
...
mortality.categorical.covars <- c('gender', 'insurance.code', 'marshall.comb', 'pt.cerebral.na',
                                  'cpr.yn', 'pupil.react.imp', 'mental.status', 'any.alpha2',
...
mortality.daily.covars <- c('max.motor.imp', 'pupil.react.imp', 'min.glucose.imp',
                            ...
                            'tot.betablock.cube', 'tot.propofol.cube', 'any.alpha2',
...
### Patient Inclusion and Missing Data {#mortinhospmissing}
...
daily.mort.inhosp.org <- tbi.daily.icu %>%
  ...
  dplyr::select(mrn, study.day, max.motor.imp, pupil.react.imp, min.glucose.imp, min.hemoglobin.imp,
                ...
                tot.propofol.cube, any.alpha2, any.prbc, any.plasmaplt)
...
label(modeldata.mort.inhosp$any.alpha2) <- label(tbi.daily$any.alpha2)
...
### Redundancy Analysis {#mortinhospredun}
...
redun.mort.inhosp <- redun(~ age + gender + insurance.code + iss + cpr.yn + marshall.comb +
                             ...
                             I(tot.propofol.cube) + any.alpha2 + any.prbc + any.plasmaplt,
...
areg.mort.inhosp <- aregImpute(~ age + gender + insurance.code + iss + cpr.yn + marshall.comb +
                                 ...
                                 tot.opioid.cube + tot.benzo.cube + tot.antipsyc.cube +
                                 tot.betablock.cube + tot.propofol.cube + any.alpha2 + any.prbc +
                                 any.plasmaplt + start.day,
...
mod.mort.inhosp <- fit.mult.impute(Surv.mort.inhosp ~ rcs(age, 4) + gender + insurance.code +
                                     ...
                                     ## can't fit splines
                                     tot.antipsyc.cube + tot.propofol.cube +
                                     ## Alpha-2 agonists and blood products have too many 0s + wide
                                     ## range; dichotomize
                                     any.alpha2 + any.prbc + any.plasmaplt,
                                   fitter = cph,
...
hrs.mort.inhosp <- hrs.mort.inhosp %>%
  ...
  mutate(flevel = factor(flevel.num, labels = hrs.mort.inhosp.flevels$comp.c),
         ...
         tipText = gsub('^ *', '',
                        gsub('\\( +', '\\(',
                             gsub('  +', ' ',
                                  paste0(comp.c, ' vs ', ref.c, ': ',
                                         rndformat(effect), ' (', rndformat(lcl), ', ',
                                         rndformat(ucl), ')')))))
...
gg.mort.inhosp <- ggplot(data = hrs.mort.inhosp,
  ...
  geom_pointrange(aes(ymin = Lower.CL, ymax = Upper.CL, colour = abs(log(HR)), text = tipText)) +
...
```{r print_mort_inhosp_figures, results = 'asis', fig.width=8.5, fig.height=75}
gg.mort.inhosp %>%
...
mort.3yr.baseline.missing <-
  as.data.frame(do.call(rbind,
                        lapply(c(mortality.baseline.covars, mortality.3yr.covars),
                               FUN = function(vname){
                                 nmiss <- sum(is.na(tbi.survivors[,vname]))
...
mort.3yr.daily.missing <- tbi.daily.icu %>%
  filter(mrn %in% pts.inc.survived) %>%
  dplyr::select(one_of(mortality.daily.covars)) %>%
  missingInDataFrame() %>%
  filter(nmiss > 0)
...
### Patient Inclusion and Missing Data {#mort3yrmissingness}
There were `r length(pts.inc.survived)` patients who survived their hospital stay and were eligible
to be included in the three-year mortality model. `r table_nums("mort_3yr_missing_baseline", display = 'cite')` details how many eligible patients are missing which baseline covariates. These missing
values will be handled using multiple imputation.

#### `r table_nums("mort_3yr_missing_baseline")`
```{r print_mort_3yr_missing_baseline, results = 'asis'}
pandoc.table(mort.3yr.baseline.missing, justify = c('left', 'right', 'right'))

```

We will calculate summary statistics for in-hospital variables across the entire hospital stay,
using multiple imputation to handle patient-days with missing data. `r table_nums("mort_3yr_missing_daily", display = 'cite')` details the percentage and number of
in-hospital patient-days among the `r length(pts.inc.survived)` patients included in the three-year
mortality model with each variable missing.
...
Summary variables will be calculated and used as follows:
...
then merge these with imputed baseline variables
...
base.survivors <- tbi.survivors %>%
  dplyr::select(mrn, age, gender, insurance.code, iss, cpr.yn, marshall.comb, pt.cerebral.na,
                base.glucose.imp, base.pupil.react.imp, base.hemoglobin.imp, base.motor.imp,
                disposition.comb, vent.days, los, fim.total, days.del.14, dcfd.14, died.3yr.dc,
                time.death.3yr.dc)

## Daily data
daily.survivors.org <- tbi.daily.icu %>%
  filter(mrn %in% pts.inc.survived) %>%
  dplyr::select(mrn, study.day, max.motor.imp, pupil.react.imp, min.glucose.imp, min.hemoglobin.imp,
                min.sodium.imp, mental.status, sofa.mod.nanormal.imp, max.icp.dich, tot.opioid,
                tot.benzo, tot.antipsyc, tot.betablock, tot.propofol, any.alpha2, any.prbc,
                any.plasmaplt)

## Combine baseline and daily variables to prep for imputation
basedaily.survivors <- left_join(base.survivors, daily.survivors.org, by = 'mrn')

## Impute daily values
mice.survivors.daily <- mice(basedaily.survivors[,-1], visitSequence = 'monotone')

## Calculate summary statistics by imputation
complete.survivors.daily <- mice::complete(mice.survivors.daily, action = 'long', include = TRUE)

summary.survivors <- complete.survivors.daily %>%
  mutate(mrn = rep(basedaily.survivors$mrn, length(unique(complete.survivors.daily$.imp)))) %>%
  group_by(.imp, mrn) %>%
  summarise(days.motor.lt6 = sum(max.motor.imp < 6),
            ...
            days.alpha2 = sum(any.alpha2 == 'One or both', na.rm = TRUE),
            ever.alpha2 = factor(as.numeric(sum(any.alpha2 == 'One or both') > 0),
            ...
            ever.plasmaplt = factor(as.numeric(sum(any.plasmaplt == 'At least one') > 0),
                                    levels = 0:1, labels = c('Never', 'At least once'))) %>%
  left_join(base.survivors, by = 'mrn') %>%
  as.data.frame()

## Add variable labels
label(summary.survivors$.imp) <- 'Imputation'
label(summary.survivors$mrn) <- 'MRN'
label(summary.survivors$days.motor.lt6) <- 'Days with max motor score <6'
label(summary.survivors$days.no.pupil.data) <- 'Days with no pupil reactivity data'
label(summary.survivors$days.fixed.pupil) <- 'Days with at least one fixed pupil'
label(summary.survivors$lowest.glucose) <- 'Lowest glucose in hospital'
label(summary.survivors$lowest.hemoglobin) <- 'Lowest hemoglobin in hospital'
label(summary.survivors$lowest.sodium) <- 'Lowest sodium in hospital'
label(summary.survivors$days.del) <- 'Days with delirium'
label(summary.survivors$days.coma) <- 'Days with coma'
label(summary.survivors$highest.modsofa) <- 'Highest modified SOFA in hospital'
label(summary.survivors$mean.opioid.cube) <- 'Mean (24h opioids), cube root'
label(summary.survivors$mean.benzo.cube) <- 'Mean (24h benzodiazepines), cube root'
label(summary.survivors$mean.antipsyc.cube) <- 'Mean (24h antipsychotics), cube root'
label(summary.survivors$mean.betablock.cube) <- 'Mean (24h beta-blockers), cube root'
label(summary.survivors$mean.propofol.cube) <- 'Mean (24h propofol), cube root'
label(summary.survivors$days.alpha2) <- 'Days received alpha-2 agonist'
label(summary.survivors$ever.alpha2) <- 'Ever received alpha-2 agonist'
label(summary.survivors$ever.prbc) <- 'Ever received PRBC'
label(summary.survivors$ever.plasmaplt) <- 'Ever received plasma, platelets and/or cryo'

## Impute baseline variables, using original delirium duration/DCFDs if available
mice.survivors.base <- mice(base.survivors[,-1], visitSequence = 'monotone')
complete.survivors.base <- mice::complete(mice.survivors.base, action = 'long', include = TRUE)

## Replace versions of daily variables which were imputed *ignoring* partial data with those
## imputed *using* partial data where available
complete.survivors.base$mrn <- rep(base.survivors$mrn, length(unique(complete.survivors.base$.imp)))

## Which variables do the completed baseline dataset and the summarized imputed dataset have in
## common? We only want the summarized daily variables to be merged; for baseline variables we'll
## take the versions from complete.survivors.base (need to keep .imp and mrn to merge)
vars.in.common <- intersect(names(complete.survivors.base), names(summary.survivors))
vars.in.common <- vars.in.common[!(vars.in.common %in% c('.imp', 'mrn'))]

complete.survivors <- left_join(complete.survivors.base,
                                dplyr::select(summary.survivors, -one_of(vars.in.common)),
                                by = c('.imp', 'mrn')) %>%
  dplyr::select(-.id) %>%
  mutate(.imp = factor(.imp))

## Put complete.survivors back into mids format to use with mice
mids.survivors <- as.mids.update(complete.survivors, .imp = 1, .id = NULL)

## Pull out original summarized data to use as starting data set
withmiss.survivors <- complete.survivors %>% filter(.imp == '0')

## Set datadist
dd.survivors <- datadist(withmiss.survivors, adjto.cat = 'first')
options(datadist = 'dd.survivors')

```

### Model Covariates {#mort3yrcovariates}
We will include the following covariates in the three-year mortality model (reference levels for
categorical variables are listed first):

- Baseline
    - Age at admission
    - Gender *(`r paste(levels(withmiss.survivors$gender), collapse = '; ')`)*
    - Insurance type *(`r paste(levels(withmiss.survivors$insurance.code), collapse = '; ')`)*
    - Marshall Class equivalent *(`r paste(levels(withmiss.survivors$marshall.comb), collapse = '; ')`)*
    - Cerebral subarachnoid hemorrhage (SAH) *(`r paste(levels(withmiss.survivors$pt.cerebral.na), collapse = '; ')`)*
    - Injurity Severity Score (ISS)
    - CPR *(`r paste(levels(withmiss.survivors$cpr.yn), collapse = '; ')`)*
- In-Hospital
    - Lowest glucose
    - Lowest hemoglobin
    - Lowest sodium
    ...
    - Days received alpha-2 agonist (clonidine and/or dexmedetomidine)
    ...
    - Whether patient ever received:
        - PRBC *(`r paste(levels(withmiss.survivors$ever.prbc), collapse = '; ')`)*
        - Plasma, platelets, and/or cryo *(`r paste(levels(withmiss.survivors$ever.plasmaplt), collapse = '; ')`)*
- Discharge
    - Days on mechanical ventilation
    - Discharge disposition *(`r paste(levels(withmiss.survivors$disposition.comb), collapse = '; ')`)*
...
### Redundancy Analysis {#mort3yrredundancy}
...
redun.mort.3yr <- redun(~ age + gender + insurance.code + iss + cpr.yn + marshall.comb +
                          ...
                          days.alpha2 + ever.prbc + ever.plasmaplt + disposition.comb + vent.days +
                          los,
                        data = withmiss.survivors,
...
mort.3yr.corr.data <- complete.survivors %>%
  filter(.imp == '1') %>%
  dplyr::select(-.imp, -mrn, -days.no.pupil.data, -ever.alpha2, -ever.prbc, -ever.plasmaplt, -gender,
                -insurance.code, -cpr.yn, -marshall.comb, -pt.cerebral.na, -disposition.comb,
                -died.3yr.dc, -time.death.3yr.dc, -base.glucose.imp, -base.pupil.react.imp,
                -base.hemoglobin.imp, -base.motor.imp)
...
```{r mort_3yr_fitmod}
Surv.mort.3yr <- with(withmiss.survivors, Surv(time = time.death.3yr.dc,
...
mod.mort.3yr <- fit.mult.impute(Surv.mort.3yr ~ rcs(age, 4) + gender + insurance.code +
                                  ...
                                  rcs(days.alpha2, 3) + ever.prbc + ever.plasmaplt +
                                  rcs(vent.days, 3) + disposition.comb,
                                fitter = cph,
                                data = withmiss.survivors,
                                xtrans = mids.survivors,
                                x = TRUE, y = TRUE)
...
a patient who was `r round(dd.survivors$limits$age[3])` years old at the time of injury has about `r round(summary(mod.mort.3yr, age = c(dd.survivors$limits$age[1], dd.survivors$limits$age[3]), est.all = FALSE)[2, 'Effect'], 1)` times the hazard of three-year death at any time point compared to a patient who was `r round(dd.survivors$limits$age[1])` years old, assuming all other covariates are equal. Where possible, the reference and comparison values for continuous variables are the 25th and 75th percentiles, respectively; for variables where these quantities are the same (eg, several drug doses), these values are the minimum and maximum.
...
```{r print_mort_3yr_figures, results = 'asis', fig.width=8.5, fig.height=95}
gg.mort.3yr %>%
  ggplotly(tooltip = c("text"))
...
delirium/coma-free days (DCFDs) during the 14 days following hospital admission vs baseline/day of
admission risk factors. We will examine relationships with DCFDs among all `r format(n.included.main, big.mark = ',')` patients in our cohort; because relationships with delirium duration could be biased by mortality, we will only look at delirium duration among those `r format(length(pts.inc.survived), big.mark = ',')` in our cohort who survived their hospital stay.
...
Missing delirium summary outcome and baseline variables will be imputed as follows (similar to our approach for the three-year mortality model, but with some differences):
...
## Delirium Duration among Hospital Survivors
### Patient Inclusion and Missing Data

```{r deldays_missing}
## -- List of baseline and day 1 covariates to include in models for delirium duration, DCFDs ------
delsummary.covars <- c(mortality.baseline.covars, 'base.motor.imp', 'base.pupil.react.imp',
...
n.deldays.missany <- sum(rowSums(is.na(tbi.survivors[,c(delsummary.covars, 'days.del.14')])) > 0)

deldays.missing <-
  as.data.frame(do.call(rbind,
                        lapply(c(delsummary.covars, 'days.del.14'),
                               FUN = function(vname){
                                 nmiss <- sum(is.na(tbi.survivors[,vname]))
                                 pctall.miss <- round((nmiss / length(pts.inc.survived))*100, 2)
                                 c(nmiss, pctall.miss)
                               })))
deldays.missing <- cbind(c(delsummary.covars, 'days.del.14'), deldays.missing)
names(deldays.missing) <- c('Variable', 'N Missing Covariate', '% Eligible Patients')
deldays.missing <- deldays.missing[deldays.missing[,2] > 0,]
rownames(deldays.missing) <- NULL

```

All `r format(length(pts.inc.survived), big.mark = ',')` hospital survivors will be included in our
model for delirium duration; `r n.deldays.missany` of these (`r round((n.deldays.missany / length(pts.inc.survived))*100)`%) are missing one or more baseline covariates and/or delirium duration. `r table_nums("deldays_missing", display = 'cite')` details how many patients in our cohort are missing which covariates or outcome. This missingness will be
handled with multiple imputation.

#### `r table_nums("deldays_missing")`
```{r print_deldays_missing, results = 'asis'}
pandoc.table(deldays.missing, justify = c('left', 'right', 'right'))
...
delsummary.rhvars <- c("age" = "rcs(age, 3)", "gender" = "gender",
                       ...
                       "base.pupil.react.imp" = "base.pupil.react.imp",
                       "base.glucose.imp" = "rcs(base.glucose.imp, 3)",
                       "base.hemoglobin.imp" = "rcs(base.hemoglobin.imp, 3)")

## -- Create data sets for delirium duration models ------------------------------------------------
## Baseline/day 0 information
base.deldays <- tbi.survivors %>%
  dplyr::select(mrn, one_of(delsummary.covars), fim.total, days.del.14)

## Daily data, for imputing delirium duration/DCFDs
daily.deldays.org <- tbi.daily.icu %>%
  filter(event %in% paste('Inpatient Day',
                          c('01', '02', '03', '04', '05', '06', '07', '08', '09', 10:14)) &
         mrn %in% tbi.survivors$mrn) %>%
  dplyr::select(mrn, study.day, max.motor.imp, pupil.react.imp, min.glucose.imp, min.hemoglobin.imp,
                min.sodium.imp, mental.status, sofa.mod.nanormal.imp, max.icp.dich, tot.opioid,
...
basedaily.deldays <- left_join(dplyr::select(base.deldays, -days.del.14), daily.deldays.org,
                               by = 'mrn')

## Impute daily values
mice.deldays.daily <- mice(basedaily.deldays[,-1], visitSequence = 'monotone')

## Create long data frame of original data + all imputed data sets
complete.deldays.daily <- mice::complete(mice.deldays.daily, action = 'long', include = TRUE)

## Add MRN and calculate summary statistics by imputation, patient
summary.deldays <- complete.deldays.daily %>%
  ...
  filter(.imp %in% 1:99) %>%
  mutate(mrn = rep(basedaily.deldays$mrn, length(unique(complete.deldays.daily$.imp)) - 1)) %>%
  group_by(.imp, mrn) %>%
...
deldays.org <- cbind(factor(rep(0, nrow(base.deldays))),
                     subset(base.deldays, select = c(mrn, days.del.14)))
names(deldays.org)[1] <- '.imp'

summary.deldays <- bind_rows(deldays.org, summary.deldays)

## Impute baseline variables, using original delirium duration/DCFDs if available
mice.deldays.base <- mice(base.deldays[,-1], visitSequence = 'monotone')
complete.deldays.base <- mice::complete(mice.deldays.base, action = 'long', include = TRUE)

## Replace versions of delirium outcomes which were imputed *ignoring* partial data with those
## imputed *using* partial data where available
complete.deldays.base$mrn <- rep(base.deldays$mrn, length(unique(complete.deldays.base$.imp)))

complete.deldays <- left_join(dplyr::select(complete.deldays.base, -days.del.14), summary.deldays,
                              by = c('.imp', 'mrn')) %>%
  dplyr::select(-.id) %>%
  mutate(.imp = factor(.imp))

## Put complete.deldays back into mids format to use with mice
mids.deldays <- as.mids.update(complete.deldays, .imp = 1, .id = NULL)
...
```{r deldays_redun, results = 'markup'}
redun.deldays <- redun(as.formula(paste('~', paste(delsummary.covars, collapse = ' + '))),
                       data = tbi.survivors, type = 'adjusted', nk = 4, allcat = TRUE)

highr2.deldays <- which.max(redun.deldays$rsq1)

round(redun.deldays$rsq1, 2)

```

The highest adjusted R^2^ value is `r round(redun.deldays$rsq1[highr2.deldays], 2)` (`r names(redun.deldays$rsq1[highr2.deldays])`), indicating that there are no redundant variables.
...
p <- ggplot(data = tbi.survivors, aes(x = days.del.14)) +
  geom_histogram(fill = "#003D79") +
  scale_x_continuous(name = 'Days of delirium within 14 days of admission', breaks = 0:14) +
...
mod.deldays <- with(mids.survivors, glm.nb(days.del.14 ~ rcs(age, 3) + gender + insurance.code +
                                           marshall.comb + pt.cerebral.na + rcs(iss, 3) + cpr.yn +
                                           base.motor.imp + base.pupil.react.imp +
...
get_irrci_deldays <- function(rhvar){
  tmp <- calc_nb_ratioci(nbObj = mod.deldays, predVar = rhvar, df = tbi.survivors) %>%
    as.data.frame()
  ...
  if(inherits(tbi.survivors[,rhvar], 'factor')){
    tmp$ref.val.c <- levels(tbi.survivors[,rhvar])[tmp$ref.val]
    tmp$comp.val.c <- levels(tbi.survivors[,rhvar])[tmp$comp.val]
  } else{
...
irrs.deldays$varLabel <- map_chr(irrs.deldays$covariate, ~label(tbi.survivors[,.]))
...
mod.deldays.null <- with(mids.deldays, glm.nb(days.del.14 ~ 1))

lrtest.deldays.overall <-
  pool.compare(mod.deldays, mod.deldays.null, data = mids.deldays, method = 'likelihood')
...
modstats.deldays <- data.frame(varLabel = c("*Observations*"),
                               ref.val.c = format(nrow(mids.deldays$data), big.mark = ','))
...
adjusted to the median or reference level of all other covariates (our best "typical" value) among
all hospital survivors in our cohort.
...
get_deldays_median <- function(x){ median(tbi.survivors[,x], na.rm = TRUE) }
get_deldays_medspline <- function(x){
  knot.locs <- rcspline.eval(tbi.survivors[,x], nk = 3, knots.only = TRUE)
  x.med <- median(tbi.survivors[,x], na.rm = TRUE)
...
create_deldays_designmatrix <- function(varName,
                                        ...
                                        df = tbi.survivors){ ## data frame to use for unique values
...
create_deldays_figures <- function(df){
  ...
  varLabel <- label(tbi.survivors[,names(df)[1]])
  ...
  if(nrow(df) <= 10){
    p <- ggplot(data = df, aes_string(x = names(df)[1], y = 'count.pe', text = 'tipText')) +
      geom_errorbar(aes(ymin = count.lcl, ymax = count.ucl), width = 0, colour = "#003D79") +
      geom_point(colour = "#003D79") +
      scale_x_continuous(name = NULL,
                         limits = c(0.5, max(df[,1]) + 0.5),
                         breaks = df[,1],
                         labels = levels(tbi.survivors[,names(df)[1]]))
  } else{
...
deldays_ggtoplotly <- function(elementNum){
  figTitle <- label(tbi.survivors[,names(deldays.ggplots)[[elementNum]]])
...
```{r deldays_figures, results = 'asis', fig.width=8.5, fig.height=15}
...
## Delirium/Coma-Free Days among All Patients
### Patient Inclusion and Missing Data

```{r dcfds_missing}
## -- How many patients are missing each covariate and outcome? ------------------------------------
## How many are missing at least one covariate/outcome?
n.dcfds.missany <- sum(rowSums(is.na(tbi.oneobs.icu[,c(delsummary.covars, 'dcfd.14')])) > 0)

dcfds.missing <-
  as.data.frame(do.call(rbind,
                        lapply(c(delsummary.covars, 'dcfd.14'),
                               FUN = function(vname){
                                 nmiss <- sum(is.na(tbi.oneobs.icu[,vname]))
                                 pctall.miss <- round((nmiss / n.included.main)*100, 2)
                                 c(nmiss, pctall.miss)
                               })))
dcfds.missing <- cbind(c(delsummary.covars, 'dcfd.14'), dcfds.missing)
names(dcfds.missing) <- c('Variable', 'N Missing Covariate', '% Eligible Patients')
dcfds.missing <- dcfds.missing[dcfds.missing[,2] > 0,]
rownames(dcfds.missing) <- NULL

```

All `r format(n.included.main, big.mark = ',')` patients in the main cohort will be
included in our model for delirium/coma-free days; `r n.dcfds.missany` of these (`r round((n.dcfds.missany / n.included.main)*100)`%) are missing one or more baseline covariates and/or the outcome. `r table_nums("dcfds_missing", display = 'cite')` details how many patients in our cohort are missing which covariates or outcome. This missingness will be handled with multiple
imputation.

#### `r table_nums("dcfds_missing")`
```{r print_dcfds_missing, results = 'asis'}
pandoc.table(dcfds.missing, justify = c('left', 'right', 'right'))
...
## -- Create data sets for delirium duration models ------------------------------------------------
## Baseline/day 0 information
base.dcfds <- tbi.oneobs.icu %>%
  dplyr::select(mrn, one_of(delsummary.covars), dcfd.14)

## Daily data, for imputing delirium duration/DCFDs
daily.dcfds.org <- tbi.daily.icu %>%
  filter(event %in% paste('Inpatient Day',
                          c('01', '02', '03', '04', '05', '06', '07', '08', '09', 10:14))) %>%
  dplyr::select(mrn, study.day, max.motor.imp, pupil.react.imp, min.glucose.imp, min.hemoglobin.imp,
                min.sodium.imp, mental.status, sofa.mod.nanormal.imp, max.icp.dich, tot.opioid,
...
## Combine baseline and daily variables to prep for daily mental status imputation
basedaily.dcfds <- left_join(dplyr::select(base.dcfds, -dcfd.14), daily.dcfds.org, by = 'mrn')

## Impute daily values
mice.dcfds.daily <- mice(basedaily.dcfds[,-1], visitSequence = 'monotone')

## Create long data frame of original data + all imputed data sets
complete.dcfds.daily <- mice::complete(mice.dcfds.daily, action = 'long', include = TRUE)

## Add MRN and calculate summary statistics by imputation, patient
summary.dcfds <- complete.dcfds.daily %>%
  ## Do not include original data for now - calculating days.del.14/dcfd.14 won't work the same way
  ## because there's no days.mental
  filter(.imp %in% 1:99) %>%
  mutate(mrn = rep(basedaily.dcfds$mrn, length(unique(complete.dcfds.daily$.imp)) - 1)) %>%
  group_by(.imp, mrn) %>%
  summarise(dcfd.14 = 14 - sum(mental.status %in% c('Delirious', 'Comatose'), na.rm = TRUE)) %>%
  as.data.frame()

## Add original data - need to add .imp first
dcfds.org <- cbind(factor(rep(0, nrow(base.dcfds))),
                   subset(base.dcfds, select = c(mrn, dcfd.14)))
names(dcfds.org)[1] <- '.imp'

summary.dcfds <- bind_rows(dcfds.org, summary.dcfds)

## Impute baseline variables, using original delirium duration/DCFDs if available
mice.dcfds.base <- mice(base.dcfds[,-1], visitSequence = 'monotone')
complete.dcfds.base <- mice::complete(mice.dcfds.base, action = 'long', include = TRUE)

## Replace versions of delirium outcomes which were imputed *ignoring* partial data with those
## imputed *using* partial data where available
complete.dcfds.base$mrn <- rep(base.dcfds$mrn, length(unique(complete.dcfds.base$.imp)))

complete.dcfds <- left_join(dplyr::select(complete.dcfds.base, -dcfd.14), summary.dcfds,
                            by = c('.imp', 'mrn')) %>%
  dplyr::select(-.id) %>%
  mutate(.imp = factor(.imp))

## Put complete.dcfds back into mids format to use with mice
mids.dcfds <- as.mids.update(complete.dcfds, .imp = 1, .id = NULL)

```

### Redundancy Analysis
To see if any of the covariates in our DCFD model are nearly fully explained by the others, we
performed a redundancy analysis. Adjusted R^2^ values for each covariate are shown below.

```{r dcfds_redun, results = 'markup'}
redun.dcfds <- redun(as.formula(paste('~', paste(delsummary.covars, collapse = ' + '))),
                     data = tbi.oneobs.icu, type = 'adjusted', nk = 4, allcat = TRUE)

highr2.dcfds <- which.max(redun.dcfds$rsq1)

round(redun.dcfds$rsq1, 2)

```

The highest adjusted R^2^ value is `r round(redun.dcfds$rsq1[highr2.dcfds], 2)` (`r names(redun.dcfds$rsq1[highr2.dcfds])`), indicating that there are no redundant variables.
All covariates will be included as specified above.

### Model Choice

The distribution of delirium/coma-free days over the first 14 days, shown below, follows a strange
distribution; therefore, we will use a proportional odds logistic regression model with multiple
imputation to look for relationships between it and baseline risk factors. POLR models are robust to
assumptions of outcome distribution.

```{r dcfd_histogram, results = 'asis'}
p <- ggplot(data = tbi.oneobs.icu, aes(x = dcfd.14)) +
  geom_histogram(fill = "#003D79") +
  theme_minimal() +
  scale_x_continuous(name = 'Delirium/Coma-Free Days', breaks = 0:14) +
  scale_y_continuous(name = 'Count of Patients', breaks = seq(0, 500, 100))
...
```{r dcfds_fitmod}
## -- Fit model for DCFDs --------------------------------------------------------------------------
mod.dcfds <- fit.mult.impute(dcfd.14 ~ rcs(age, 3) + gender + insurance.code + marshall.comb +
                               pt.cerebral.na + rcs(iss, 3) + cpr.yn + base.motor.imp +
                               base.pupil.react.imp + rcs(base.glucose.imp, 3) +
                               rcs(base.hemoglobin.imp, 3),
                             fitter = lrm,
                             xtrans = mids.dcfds,
                             data = tbi.oneobs.icu,
...
## -- Set datadist ---------------------------------------------------------------------------------
dd.dcfds <- datadist(tbi.oneobs.icu)
options(datadist = 'dd.dcfds')

```

### Proportional Odds Assumption
```{r dcfds_assumptions}
p <- rms_po_assume(mod.dcfds, cuts = 1:13, mfrowAuto = FALSE,
                   impObj = mids.dcfds, modelData = tbi.oneobs.icu) +
  theme(strip.text = element_text(size = 7),
        axis.text.x = element_text(size = 5))

```

```{r print_dcfds_assumptions, results = 'asis', fig.width = 8.5, fig.height = 6}
p %>%
  ggplotly(tooltip = c('x', 'y'))
...
### Model Results
`r table_nums('dcfds_results', display = 'cite')` presents a summary of results from the
delirium/coma-free days model. Odds ratios indicate a comparison between the "comparison" vs the "reference" columns, holding all other covariates at the median or reference category. For example,
a patient who was `r round(dd.dcfds$limits$age[3])` years old at the time of injury has about `r round(summary(mod.dcfds, age = c(dd.dcfds$limits$age[1], dd.dcfds$limits$age[3]), est.all = FALSE)[2, 'Effect'], 1)` times the odds of more DCFDs compared to a patient who was `r round(dd.dcfds$limits$age[1])` years old, assuming all other covariates are equal. Reference and comparison values for continuous variables are the 25th and 75th percentiles, respectively.

**Please note** that while p-values reflect the entire association of each covariate with our
outcome, odds ratios are only valid for the comparisons listed: For continuous covariates which are allowed to have a nonlinear association with DCFDs, the odds ratios depend on the two values we choose; for categorical covariates with more than two categories, the odds ratios depend on the reference level chosen.

**Reminder**: For delirium/coma-free days, an odds ratio above 1 is **good** (higher odds of more
"good" days, alive with normal mental status); an odds ratio below 1 is **bad** (higher odds of more
"bad" days with delirium, coma, and/or death).

#### `r table_nums('dcfds_results')`
```{r dcfds_results}
results.dcfds <- rms_model_results(mod.dcfds, labeldf = tbi.oneobs.icu, printFormat = 'markdown')
names(results.dcfds) <- gsub('HR ', 'OR ', cph.results.headers)

```

```{r print_dcfds_results, results = 'asis'}
pandoc.table(results.dcfds[1:(nrow(results.dcfds) - 15),],
             justify = 'lrrrrrr', style = 'rmarkdown', split.tables = 800)
```

#### `r figure_nums('dcfds_figures')`
For numeric variables, all odds ratios are for comparison to the median, our best "typical" value;
comparison values are the 10th, 25th, 75th, and 90th percentiles by default, though in some cases, some of these percentiles are the same. For categorical variables, the reference level is the lefthand level.
...
```{r dcfds_figures}
## -- Get data frame of all ORs for all covariates -------------------------------------------------
## Get all unique variables used in model fit
vars.dcfds <- unique(gsub("'+|=.+$", "", names(mod.dcfds$coefficients)))[-1]

ors.dcfds <- map(vars.dcfds, rms_calc_comparisons,
                 rmsObj = mod.dcfds, getRatios = TRUE, df = tbi.oneobs.icu) %>%
  bind_rows()

## -- Add variable *labels* ------------------------------------------------------------------------
ors.dcfds.labels <- map_df(vars.dcfds,
                           ~data.frame('variable' = ., 'varlabel' = label(tbi.oneobs.icu[,.]))) %>%
  bind_rows()

ors.dcfds <- ors.dcfds %>%
  left_join(ors.dcfds.labels, by = 'variable') %>%
  left_join(dplyr::select(results.dcfds, Covariate, P),
            by = c("varlabel" = "Covariate"))

## -- Create properly ordered factor levels --------------------------------------------------------
ors.dcfds.flevels <-
  ## Get all unique factor levels + whether they are reference
  unique(subset(ors.dcfds, select = c(comp.c, is.ref))) %>%
  ## Create variables by which to order: numeric/NA, then for character, whether reference
  ## (numeric levels will always be ordered by numeric value, regardless of reference)
  mutate(level.num = as.numeric(as.character(comp.c)),
         is.ref.factor = ifelse(!is.na(level.num), FALSE, is.ref)) %>%
...
## Merge numeric values onto original data set, create final factor variable
ors.dcfds <- ors.dcfds %>%
  left_join(dplyr::select(ors.dcfds.flevels, comp.c, flevel.num),
            by = c('comp.c')) %>%
  mutate(flevel = factor(flevel.num, labels = ors.dcfds.flevels$comp.c),
         varlabel.p = paste0(varlabel, '; P: ', P),
         tipText = gsub('^ *', '',
                        gsub('\\( +', '\\(',
                             gsub('  +', ' ',
...
## -- Create a single faceted plot for all covariate HRs vs reference levels -----------------------
## Change column names to work better in tooltips
names(ors.dcfds) <- gsub('flevel', 'Comparison', names(ors.dcfds))
names(ors.dcfds) <- gsub('lcl', 'Lower.CL', names(ors.dcfds))
names(ors.dcfds) <- gsub('ucl', 'Upper.CL', names(ors.dcfds))
names(ors.dcfds) <- gsub('effect', 'OR', names(ors.dcfds))
names(ors.dcfds) <- gsub('ref.c', 'Reference', names(ors.dcfds))

gg.dcfds <- ggplot(data = ors.dcfds, aes(x = Comparison, label = Reference, y = OR)) +
  facet_wrap(~ varlabel.p, ncol = 1, scales = 'free') +
  geom_hline(yintercept = 1, colour = 'grey90', size = 1.25) +
  geom_pointrange(aes(ymin = Lower.CL, ymax = Upper.CL, colour = abs(log(OR)), text = tipText)) +
  scale_color_viridis(direction = -1, end = 0.9, name = 'Effect Size<br>(Log Scale)') +
  scale_x_discrete(name = '') +
  scale_y_continuous(name = ' <br>Odds Ratios (95% Confidence Intervals), Delirium/Coma-Free Days<br> ',
                     breaks = c(0.25, 0.5, 1, 1.25, 1.5, 2, 4, 8, 16)) +
  guides(fill = FALSE) +
  theme_bw() +
  theme(axis.text = element_text(size = 8))
...
```{r print_dcfds_figures, results = 'asis', fig.width=8.5, fig.height=35}
gg.dcfds %>%
  ggplotly(tooltip = c("text"))

```

# Functional Independence Measure among Hospital Survivors

The [Functional Independence Measure](https://en.wikipedia.org/wiki/Functional_Independence_Measure) is a score which, in our case, describes disability and burden of care among TBI survivors.
**Higher** scores are **better**.

## Patient Inclusion and Missing Data

```{r fim_missing}
nMissFIM <- sum(is.na(tbi.survivors$fim.total))
pctMissFIM <- round(mean(is.na(tbi.survivors$fim.total))*100)

```

All `r length(pts.inc.survived)` survivors in our cohort are included in this model, which will use
the [same baseline and in-hospital covariates](#mort3yrcovariates) as the three-year mortality
model. Please see previous sections for details on [covariate missingness](#mort3yrmissingness) and [redundancy](#mort3yrredundancy) among these covariates. Among survivors, `r pctMissFIM`% (N = `r nMissFIM`) were missing FIM score at hospital discharge. Missing covariates and outcomes will be handled using multiple imputation.

## Model Choice

The distribution of FIM scores at discharge, shown below, follows a distribution not well described
by a mathematical distribution; therefore, we will use a proportional odds logistic regression model
with multiple imputation to look for relationships between it and baseline risk factors. POLR models
are robust to assumptions of outcome distribution.

```{r fim_histogram, results = 'asis', message = FALSE}
p <- ggplot(data = subset(tbi.survivors, !is.na(fim.total)), aes(x = fim.total)) +
  geom_histogram(fill = "#003D79") +
  theme_minimal() +
  scale_x_continuous(name = 'Functional Independence Measure at Discharge', breaks = 0:14) +
  scale_y_continuous(name = 'Count of Patients', breaks = seq(0, 500, 100))
...
## -- Fit model for FIM score ----------------------------------------------------------------------
mod.fim <- fit.mult.impute(fim.total ~ rcs(age, 4) + gender + insurance.code + rcs(iss, 4) +
                             cpr.yn + marshall.comb + pt.cerebral.na + days.motor.lt6 +
                             days.fixed.pupil + rcs(days.del, 3) + rcs(days.coma, 3) +
                             rcs(highest.modsofa, 3) + rcs(lowest.glucose, 4) +
                             rcs(lowest.hemoglobin, 4) + rcs(lowest.sodium, 4) +
                             rcs(mean.opioid.cube, 3) + rcs(mean.benzo.cube, 3) +
                             mean.antipsyc.cube + rcs(mean.betablock.cube, 3) +
                             rcs(mean.propofol.cube, 4) + rcs(days.alpha2, 3) + ever.prbc +
                             ever.plasmaplt + rcs(vent.days, 3) + disposition.comb,
                           fitter = lrm,
                           xtrans = mids.survivors,
                           data = withmiss.survivors,
                           x = TRUE, y = TRUE)
...
### Proportional Odds Assumption
```{r fim_assumptions}
p <- rms_po_assume(mod.fim, cuts = 4:11, mfrowAuto = FALSE,
                   impObj = mids.survivors, modelData = withmiss.survivors) +
  theme(strip.text = element_text(size = 7),
        axis.text.x = element_text(size = 5))

```

```{r print_fim_assumptions, results = 'asis', fig.width = 8.5, fig.height = 6}
p %>%
  ggplotly(tooltip = c('x', 'y'))

```

In the original model, the restricted cubic spline for highest modified SOFA was fit using four
knots (three degrees of freedom), but the last coefficient did not satisfy the assumption of proportional odds, varying dramatically over the levels of FIM scores; in other words, it seems that
this four-knot spline was too unstable to be reliable. We refit the model using only three knots for
this covariate, and the proportional odds assumption is met satisfactorily.

## Model Results
`r table_nums('fim_results', display = 'cite')` presents a summary of results from the model for FIM
score at discharge. Odds ratios indicate a comparison between the "comparison" vs the "reference" columns, holding all other covariates at the median or reference category. For example,
a patient who was `r round(dd.survivors$limits$age[3])` years old at the time of injury has about `r round(summary(mod.fim, age = c(dd.survivors$limits$age[1], dd.survivors$limits$age[3]), est.all = FALSE)[2, 'Effect'], 1)` times the odds of a higher FIM score compared to a patient who was `r round(dd.survivors$limits$age[1])` years old, assuming all other covariates are equal. Reference and comparison values for continuous variables are the 25th and 75th percentiles, respectively.

**Please note** that while p-values reflect the entire association of each covariate with our
outcome, odds ratios are only valid for the comparisons listed: For continuous covariates which are allowed to have a nonlinear association with FIM scores, the odds ratios depend on the two values we choose; for categorical covariates with more than two categories, the odds ratios depend on the reference level chosen.

**Reminder**: For FIM scores, an odds ratio above 1 is **good** (higher odds of a higher score,
i.e., more independence); an odds ratio below 1 is **bad** (higher odds of a lower score, i.e., less independence).

### `r table_nums('fim_results')`
```{r fim_results}
results.fim <- rms_model_results(mod.fim, labeldf = summary.survivors, printFormat = 'markdown')
names(results.fim) <- gsub('HR ', 'OR ', cph.results.headers)

```

```{r print_fim_results, results = 'asis'}
pandoc.table(results.fim[1:(nrow(results.fim) - 10),],
             justify = 'lrrrrrr', style = 'rmarkdown', split.tables = 800)
```

### `r figure_nums('fim_figures')`
For numeric variables, all odds ratios are for comparison to the median, our best "typical" value;
comparison values are the 10th, 25th, 75th, and 90th percentiles by default, though in some cases, some of these percentiles are the same. For categorical variables, the reference level is the lefthand level.

**Reminder**: For FIM scores, an odds ratio above 1 is **good** (higher odds of "more independence"); an odds ratio below 1 is **bad** (higher odds of "less independence").

```{r fim_figures}
## -- Get data frame of all ORs for all covariates -------------------------------------------------
## Get all unique variables used in model fit
vars.fim <- unique(gsub("'+|=.+$", "", names(mod.fim$coefficients)))[-1]

ors.fim <- map(vars.fim, rms_calc_comparisons,
               rmsObj = mod.fim, getRatios = TRUE, df = summary.survivors) %>%
  bind_rows()

## -- Add variable *labels* ------------------------------------------------------------------------
ors.fim.labels <- map_df(vars.fim,
                         ~data.frame('variable' = ., 'varlabel' = label(summary.survivors[,.]))) %>%
  bind_rows()

ors.fim <- ors.fim %>%
  left_join(ors.fim.labels, by = 'variable') %>%
  left_join(dplyr::select(results.fim, Covariate, P),
            by = c("varlabel" = "Covariate"))

## -- Create properly ordered factor levels --------------------------------------------------------
ors.fim.flevels <-
  ## Get all unique factor levels + whether they are reference
  unique(subset(ors.fim, select = c(comp.c, is.ref))) %>%
  ## Create variables by which to order: numeric/NA, then for character, whether reference
  ## (numeric levels will always be ordered by numeric value, regardless of reference)
  mutate(level.num = as.numeric(as.character(comp.c)),
         is.ref.factor = ifelse(!is.na(level.num), FALSE, is.ref)) %>%
...
## Merge numeric values onto original data set, create final factor variable
ors.fim <- ors.fim %>%
  left_join(dplyr::select(ors.fim.flevels, comp.c, flevel.num),
            by = c('comp.c')) %>%
  mutate(flevel = factor(flevel.num, labels = ors.fim.flevels$comp.c),
         varlabel.p = paste0(varlabel, '; P: ', P),
         tipText = gsub('^ *', '',
                        gsub('\\( +', '\\(',
                             gsub('  +', ' ',
...
## -- Create a single faceted plot for all covariate HRs vs reference levels -----------------------
## Change column names to work better in tooltips
names(ors.fim) <- gsub('flevel', 'Comparison', names(ors.fim))
names(ors.fim) <- gsub('lcl', 'Lower.CL', names(ors.fim))
names(ors.fim) <- gsub('ucl', 'Upper.CL', names(ors.fim))
names(ors.fim) <- gsub('effect', 'OR', names(ors.fim))
names(ors.fim) <- gsub('ref.c', 'Reference', names(ors.fim))

gg.fim <- ggplot(data = ors.fim, aes(x = Comparison, label = Reference, y = OR)) +
  facet_wrap(~ varlabel.p, ncol = 1, scales = 'free') +
  geom_hline(yintercept = 1, colour = 'grey90', size = 1.25) +
  geom_pointrange(aes(ymin = Lower.CL, ymax = Upper.CL, colour = abs(log(OR)), text = tipText)) +
  scale_color_viridis(direction = -1, end = 0.9, name = 'Effect Size<br>(Log Scale)') +
  scale_x_discrete(name = '') +
  scale_y_continuous(name = ' <br>Odds Ratios (95% Confidence Intervals), FIM Score at Discharge<br> ',
                     breaks = c(0.25, 0.5, 1, 1.25, 1.5, 2, 4, 8, 16)) +
  guides(fill = FALSE) +
  theme_bw() +
  theme(axis.text = element_text(size = 8))
...
```{r print_fim_figures, results = 'asis', fig.width=8.5, fig.height=35}
gg.fim %>%
  ggplotly(tooltip = c("text"))
...
To identify potential predictors of time to hospital discharge (~ hospital length of stay) among
all patients in our cohort, we will use Cox proportional hazards regression, including the same
baseline and time-dependent covariates used for [in-hospital mortality](#mortinhospcovariates).
Again, due to the complexity of imputing both baseline and daily covariates in a time-dependent
model, we will exclude the `r n.exc.mort.inhosp` patients missing baseline covariates. Please see
previous sections for detailed information on [missing covariates](#mortinhospmissing) and [covariate redundancy](#mortinhospredun).

```{r hospdc_datamgmt}
## -- Create data set for time to discharge model --------------------------------------------------
## Create beginning time-varying data set
tv.hospdc <- create_countprocess_data(org.data = daily.mort.inhosp.org,
                                      id.var = 'mrn',
                                      record.var = 'study.day',
                                      time.var = 'time.death.dc',
                                      event.var = 'hosp.death',
                                      event.string = 'No',
                                      data.set = tbi.oneobs.icu,
                                      out.strings = c('Still in hospital', 'Discharged today'))

modeldata.hospdc <- tv.hospdc %>%
  ## Replace -9999s with NAs, refactor applicable variables
  mutate(max.motor.imp = set.backto.na(max.motor.imp),
         pupil.react.imp = factor(set.backto.na(pupil.react.imp),
                                  levels = 1:3, labels = levels(tbi.daily$pupil.react.imp)),
...
## For some reason, labels for some variables are not preserved. Add them again. Sigh.
label(modeldata.hospdc$max.motor.imp) <- label(tbi.daily$max.motor.imp)
label(modeldata.hospdc$pupil.react.imp) <- label(tbi.daily$pupil.react.imp)
label(modeldata.hospdc$min.glucose.imp) <- label(tbi.daily$min.glucose.imp)
label(modeldata.hospdc$min.hemoglobin.imp) <- label(tbi.daily$min.hemoglobin.imp)
label(modeldata.hospdc$min.sodium.imp) <- label(tbi.daily$min.sodium.imp)
label(modeldata.hospdc$mental.status) <- label(tbi.daily$mental.status)
label(modeldata.hospdc$max.icp.dich) <- label(tbi.daily$max.icp.dich)
label(modeldata.hospdc$any.alpha2) <- label(tbi.daily$any.alpha2)
label(modeldata.hospdc$any.prbc) <- label(tbi.daily$any.prbc)
label(modeldata.hospdc$any.plasmaplt) <- label(tbi.daily$any.plasmaplt)
label(modeldata.hospdc$pupil.react.imp) <- label(tbi.daily$pupil.react.imp)
## Shorten super long label for modified SOFA
label(modeldata.hospdc$sofa.mod.nanormal.imp) <- 'Modified SOFA (missing comp. = 0)'

dd.hospdc <- datadist(modeldata.hospdc, adjto.cat = 'first')
options(datadist = 'dd.hospdc')
...
## -- Imputation for in-hospital mortality ---------------------------------------------------------
set.seed(56)
areg.hospdc <- aregImpute(~ age + gender + insurance.code + iss + cpr.yn + marshall.comb +
                            pt.cerebral.na + hosp.death + I(max.motor.imp) + pupil.react.imp +
                            min.glucose.imp + min.hemoglobin.imp + min.sodium.imp +
                            mental.status + sofa.mod.nanormal.imp + max.icp.dich +
                            tot.opioid.cube + tot.benzo.cube + tot.antipsyc.cube +
                            tot.betablock.cube + tot.propofol.cube + any.alpha2 + any.prbc +
                            any.plasmaplt + start.day,
                          data = modeldata.hospdc,
                          n.impute = 5, burnin = 3)

## -- Fit model for hospital discharge -------------------------------------------------------------
Surv.hospdc <- with(modeldata.hospdc, {
  Surv(time = start.day, time2 = stop.day, event = as.numeric(died)) })

mod.hospdc <- fit.mult.impute(Surv.hospdc ~ rcs(age, 4) + gender + insurance.code +
                                rcs(iss, 4) + cpr.yn + marshall.comb + pt.cerebral.na +
                                max.motor.imp + pupil.react.imp + rcs(min.glucose.imp, 4) +
                                rcs(min.hemoglobin.imp, 4) + rcs(min.sodium.imp, 4) +
                                mental.status + rcs(sofa.mod.nanormal.imp, 4) +
...
```{r hospdc_checkph, results='hide'}
## -- Check proportional hazard assumption ---------------------------------------------------------
zph.hospdc <- cox.zph(mod.hospdc, transform = 'log')

par(mfrow = c(1, 2))
plot(zph.hospdc, col = 'red', lwd = 2)
abline(v = 0, lty = 2, col = 'gray')
par(mfrow = c(1, 1))
...
## Model Results
`r table_nums('hospdc_results', display = 'cite')` presents a summary of results from the
time to hospital discharge model. Hazard ratios indicate a comparison between the "comparison" vs
the "reference" columns, holding all other covariates at the median or reference category. For example, a patient who was `r round(dd.hospdc$limits$age[3])` years old at the time of injury has about `r round(summary(mod.hospdc, age = c(dd.hospdc$limits$age[1], dd.hospdc$limits$age[3]), est.all = FALSE)[2, 'Effect'], 1)` times the hazard of discharge at any time point compared to a patient who was `r round(dd.hospdc$limits$age[1])` years old, assuming all other covariates are equal. Where possible, the reference and comparison values for continuous variables are the 25th and 75th percentiles, respectively; for variables where these quantities are the same (eg, several drug doses), these values are the minimum and maximum.

**Please note** that while p-values reflect the entire association of each covariate with our
outcome, hazard ratios are only valid for the comparisons listed: For continuous covariates which are allowed to have a nonlinear association with time to discharge, the hazard ratios depend on the
two values we choose; for categorical covariates with more than two categories, the hazard ratios depend on the reference level chosen. More detailed hazard ratios are displayed in `r figure_nums('hospdc_figures', display = 'cite')`.

### `r table_nums('hospdc_results')`
```{r hospdc_results}
results.hospdc <- rms_model_results(mod.hospdc,
                                    labeldf = modeldata.hospdc,
                                    printFormat = 'markdown')
names(results.hospdc) <- cph.results.headers

```

```{r print_hospdc_results, results = 'asis'}
pandoc.table(results.hospdc, justify = 'lrrrrrr', style = 'rmarkdown', split.tables = 800)
```

#### `r figure_nums('hospdc_figures')`
For numeric variables, all hazard ratios are for comparison to the median, our best "typical" value;
comparison values are the 10th, 25th, 75th, and 90th percentiles by default, though in some cases
(drug doses, for example), some of these percentiles are the same. For categorical variables, the
reference level is the lefthand level.
...
## -- Get data frame of all HRs for all covariates -------------------------------------------------
## Get all unique variables used in model fit
vars.hospdc <- unique(gsub("'+|=.+$", "", names(mod.hospdc$coefficients)))

hrs.hospdc <- bind_rows(lapply(vars.hospdc,
                               FUN = rms_calc_comparisons,
                               rmsObj = mod.hospdc,
                               getRatios = TRUE,
                               df = modeldata.hospdc))
...
## -- Add variable *labels* ------------------------------------------------------------------------
hrs.hospdc.labels <- as.data.frame(do.call(rbind,
                                           lapply(vars.hospdc, FUN = function(v){
                                             c('variable' = v,
                                               'varlabel' = label(modeldata.hospdc[,v]))
                                           })))
hrs.hospdc <- hrs.hospdc %>%
  left_join(hrs.hospdc.labels, by = 'variable') %>%
  left_join(dplyr::select(results.hospdc, Covariate, P),
            by = c("varlabel" = "Covariate"))

## -- Create properly ordered factor levels --------------------------------------------------------
hrs.hospdc.flevels <-
  ## Get all unique factor levels + whether they are reference
  unique(subset(hrs.hospdc, select = c(comp.c, is.ref))) %>%
  ## Create variables by which to order: numeric/NA, then for character, whether reference
  ## (numeric levels will always be ordered by numeric value, regardless of reference)
  mutate(level.num = as.numeric(as.character(comp.c)),
         is.ref.factor = ifelse(!is.na(level.num), FALSE, is.ref)) %>%
...
## Merge numeric values onto original data set, create final factor variable
hrs.hospdc <- hrs.hospdc %>%
  left_join(dplyr::select(hrs.hospdc.flevels, comp.c, flevel.num),
            by = c('comp.c')) %>%
  mutate(flevel = factor(flevel.num, labels = hrs.hospdc.flevels$comp.c),
         varlabel.p = paste0(varlabel, '; P: ', P),
         tipText = gsub('^ *', '',
                        gsub('\\( +', '\\(',
                             gsub('  +', ' ',
...
## -- Create a single faceted plot for all covariate HRs vs reference levels -----------------------
## Change column names to work better in tooltips
names(hrs.hospdc) <- gsub('flevel', 'Comparison', names(hrs.hospdc))
names(hrs.hospdc) <- gsub('lcl', 'Lower.CL', names(hrs.hospdc))
names(hrs.hospdc) <- gsub('ucl', 'Upper.CL', names(hrs.hospdc))
names(hrs.hospdc) <- gsub('effect', 'HR', names(hrs.hospdc))
names(hrs.hospdc) <- gsub('ref.c', 'Reference', names(hrs.hospdc))

gg.hospdc <- ggplot(data = hrs.hospdc, aes(x = Comparison, label = Reference, y = HR)) +
  facet_wrap(~ varlabel.p, ncol = 1, scales = 'free') +
  geom_hline(yintercept = 1, colour = 'grey90', size = 1.25) +
  geom_pointrange(aes(ymin = Lower.CL, ymax = Upper.CL, colour = abs(log(HR)), text = tipText)) +
  scale_color_viridis(direction = -1, end = 0.9, name = 'Effect Size<br>(Log Scale)') +
  scale_x_discrete(name = '') +
  scale_y_continuous(name = ' <br>Hazard Ratios (95% Confidence Intervals), Time to Hospital Discharge<br> ',
                     breaks = c(0.25, 0.5, 1, 1.25, 1.5, 2, 4, 8, 16)) +
  guides(fill = FALSE) +
  theme_bw() +
  theme(axis.text = element_text(size = 8))
...
```{r print_hospdc_figures, results = 'asis', fig.width=8.5, fig.height=75}
gg.hospdc %>%
  ggplotly(tooltip = c("text"))
...
```{r save_image}
save.image(file = paste0('tbimort_', Sys.Date(), '.Rdata'))
```
