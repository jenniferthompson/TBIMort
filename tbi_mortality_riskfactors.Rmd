...
table_nums(name = 'dcfds_results', caption = 'Model Results Summary, Delirium Duration')
...
label(summary.mort.3yr$days.alpha2) <- 'Days received alpha-2 agonist'
label(summary.mort.3yr$ever.alpha2) <- 'Ever received alpha-2 agonist'
...
## -- Create data set of only hospital survivors to use for delirium duration model ----------------
...
base.deldays <- tbi.survivors %>%
  dplyr::select(mrn, one_of(delsummary.covars), days.del.14)
...
mjs_plot(tbi.survivors$days.del.14,
         title = '14-Day Delirium Duration among Hospital Survivors',
...
mod.deldays <- with(mids.deldays, glm.nb(days.del.14 ~ rcs(age, 3) + gender + insurance.code +
...
# mjs_plot(tbi.oneobs.icu$dcfd.14,
#          title = '14-Day Delirium/Coma-Free Days among All Patients',
#          description = 'Histogram of delirium/coma-free days within 14 days of hospital admission. Each bar = number of DCFDs; barh eight = counts of patients with that many DCFDs.',
#          decimals = 0,
#          width = 750, height = 400) %>%
#   mjs_histogram(bins = 15) %>%
#   mjs_labs(x_label = 'DCFDs within 14 days of admission',
#            y_label = 'Count')
...
mod.dcfds <- fit.mult.impute(dcfd.14 ~ rcs(age, 3) + gender + insurance.code + marshall.comb +
                               ...
                               base.pupil.react + rcs(base.glucose.imp, 3) +
                               rcs(base.hemoglobin.imp, 3),
                             fitter = lrm,
                             xtrans = mids.dcfds,
                             data = tbi.oneobs.icu,
...
mod.dcfds.noimp <- lrm(dcfd.14 ~ rcs(age, 3) + gender + insurance.code + marshall.comb +
                         pt.cerebral.na + rcs(iss, 3) + cpr.yn + base.motor.imp +
                         base.pupil.react + rcs(base.glucose.imp, 3) +
                         rcs(base.hemoglobin.imp, 3),
                       data = tbi.oneobs.icu,
