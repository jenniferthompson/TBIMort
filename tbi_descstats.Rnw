...
describe.missing <- function(dataset, var){
  ...
  if('factor' %in% class(dataset[,var]) | 'character' %in% class(dataset[,var])){
    ...
    var.table$Var1 <- with(var.table, ifelse(is.na(Var1), 'No Data', as.character(Var1)))
    ...
    results.df <- rbind(subset(var.table, Var1 == 'No Data', select = c(Var1, npct)),
                        c(rep('', 2)),
                        subset(var.table, Var1 != 'No Data', select = c(Var1, npct)))
    ## Numeric variable
  } else{
    ...
    results.df <- data.frame(Var1 = c('No Data', '', 'Median (IQR)', 'Mean (SD)'),
...
deldur.miss <- do.call(rbind,
                       lapply(c(baseline.vars, 'days.del'),
                              FUN = describe.missing, dataset = tbi.oneobs))
